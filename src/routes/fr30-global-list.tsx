...
import {
  FR30_DOMAINS,
  FR30_GLOBAL_META,
  FR30_GLOBAL_PRINCIPLES,
  type Fr30Person,
} from "@/lib/fr30-global";

/**
 * FR30 — GLOBAL EDITION · 2026. The publication page.
 *
 ...
 *
 * The thirty people and every word about them live in src/lib/fr30-global.ts;
 * this file is only the composition. The portraits are the normalised 4:5
 * series generated from the supplied FR30/df-global set — five formats and
...
export const Route = createFileRoute("/fr30-global-list")({
  head: () => ({
    meta: [
      { title: "Global FR30 — 2026 | Financial Rails" },
      {
        name: "description",
        content:
          "The inaugural Global FR30: thirty people building the infrastructure money moves through, across core infrastructure, inclusion, regulation, institutional adoption, capital markets and financial intelligence. Published 15 August 2026.",
      },
      { property: "og:title", content: "Global FR30 — 2026 | Financial Rails" },
      {
...
function Portrait({ person }: { person: Fr30Person }) {
 ...
 */
function ProfileCard({ person, delay }: { person: Fr30Person; delay: number }) {
  return (
    <Reveal delay={delay}>
...
function Fr30GlobalList() {
  return (
    <>
      ...
      {/* 03 — THE FR30 · thirty profiles in six named domains. Each domain
      ...
      <Section label={FR30_GLOBAL_META.listLabel}>
        <Reveal>
          <h2 className="display-lg max-w-[20ch]">{FR30_GLOBAL_META.listLabel}.</h2>
        </Reveal>

        {FR30_DOMAINS.map((domain) => (
      ...
      <Section label="How the FR30 Is Built" tone="bone">
        <div className="grid gap-y-12 lg:grid-cols-12 lg:gap-x-12">
          <div className="min-w-0 lg:col-span-5">
            <Reveal>
              <h2 className="display-lg max-w-[15ch]">{FR30_GLOBAL_META.methodHeading}</h2>
            </Reveal>
            <Reveal delay={110}>
              <p className="mt-8 max-w-[50ch] text-base leading-relaxed opacity-75">
                {FR30_GLOBAL_META.method}
              </p>
            </Reveal>
          </div>
          <div className="min-w-0 lg:col-span-6 lg:col-start-7">
            <div className="grid gap-x-10 gap-y-8 sm:grid-cols-2">
              {FR30_GLOBAL_PRINCIPLES.map((principle, i) => (
        ...
        <div className="py-4 lg:py-10">
          <Reveal>
            <p className="font-display text-[clamp(1.35rem,calc((100vw-48px)/12.2),5.6rem)] font-extrabold uppercase leading-[0.86] tracking-[-0.035em] md:text-[clamp(1.35rem,calc((100vw-96px)/12.2),5.6rem)] lg:text-[clamp(1.35rem,calc((100vw-224px)/12.2),5.6rem)]">
              {FR30_GLOBAL_META.closeHeadline[0]}
              <br />
              {FR30_GLOBAL_META.closeHeadline[1]}
            </p>
          </Reveal>
          <Reveal delay={120}>
            <p className="display-sm mt-10 max-w-[34ch] opacity-80">{FR30_GLOBAL_META.closeLine}</p>
          </Reveal>
          <Reveal delay={200} className="mt-14 border-t border-hairline-invert pt-8">
            <p className="label accord-signal-invert">{FR30_GLOBAL_META.signature}</p>
          </Reveal>
        </div>
      </Section>
    </>
