    ...
    <DSSection chapter="07">
      <div className="grid gap-y-9 lg:grid-cols-12 lg:gap-x-14">
          ...
          <Reveal delay={80}>
            <div className="mt-8 border-t border-hairline">
              {MEET.groups.map((group, i) => (
                <div
                  key={group.role}
                  className="grid grid-cols-[2.6rem_1fr] items-baseline gap-x-4 border-b border-hairline py-5 lg:py-6"
                >
                  <p className="label-lg accord-signal">{String(i + 1).padStart(2, "0")}</p>
                  <div className="min-w-0">
                    <h3 className={CARD_TITLE}>{group.role}</h3>
                    <p className={cn(SUPPORT, "mt-1.5 max-w-[52ch]")}>{group.line}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <Reveal delay={120} className="min-w-0 lg:col-span-5">
          <figure className="relative aspect-[4/3] w-full overflow-hidden bg-bone lg:sticky lg:top-24 lg:aspect-[4/5]">
